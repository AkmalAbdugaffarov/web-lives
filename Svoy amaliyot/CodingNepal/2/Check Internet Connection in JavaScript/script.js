...
const checkConnection = async () => {
    try {
        // Try to fetch random data from the API. If the status code is between 
				//(API dan tasodifiy ma'lumotlarni olishga harakat qiling. Agar holat kodi o'rtasida bo'lsa)
        // 200 and 300, the network connection is considered online
				//(200 va 300, tarmoq ulanishi onlayn hisoblanadi) 
        const response = await fetch("https://jsonplaceholder.typicode.com/posts");
        isOnline = response.status >= 200 && response.status < 300;
    } catch (error) {
        isOnline = false; // If there is an error, the connection is considered offline
    }											//(Agar xato bo'lsa, ulanish oflayn deb hisoblanadi)
    timer = 10;
...
const handlePopup = (status) => {
    if(status) { // If the status is true (online), update icon, title, and description accordingly
        wifiIcon.className = "uil uil-wifi";//(Agar holat rost (onlayn) boʻlsa, belgi, sarlavha va tavsifni mos ravishda yangilang)
    ...
    // If the status is false (offline), update the icon, title, and description accordingly
		//(Agar holat noto'g'ri bo'lsa (oflayn), belgi, sarlavha va tavsifni mos ravishda yangilang)
    ...
    intervalId = setInterval(() => { // Set an interval to decrease the timer by 1 every second
        timer--;										 //(Taymerni har soniyada 1 ga kamaytirish uchun intervalni o'rnating)
        if(timer === 0) checkConnection(); // If the timer reaches 0, check the connection again
        popup.querySelector(".desc b").innerText = timer;//(Taymer 0 ga yetsa, ulanishni yana tekshiring)
    }, 1000);
}

// Only if isOnline is true, check the connection status every 3 seconds
//(Faqat isOnline rost boʻlsa, har 3 soniyada ulanish holatini tekshiring)
